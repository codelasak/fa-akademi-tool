...
export default function PaymentsPage() {
  ...
  const [editingPayment, setEditingPayment] = useState<SchoolPayment | null>(
    null,
  );
  ...
  const handleCreatePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "error",
          text: errorData.error || "Ödeme kaydı oluşturulamadı",
        });
      }
    } catch (error) {
      ...
      setMessage({
        type: "error",
        text: "Ödeme kaydı oluşturulurken hata oluştu",
      });
    }
  };
  ...
  const handleUpdatePayment = async (paymentId: string, updates: any) => {
    try {
      const response = await fetch(
        `/api/admin/finances/payments/${paymentId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="rounded-lg bg-white p-6 shadow-card dark:bg-gray-dark">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
          ...
          <div className="flex items-end">
            <button
              onClick={() =>
                setFilters({ month: "", year: "", status: "", schoolId: "" })
              }
              ...
              onSubmit={(e) => {
                ...
                handleUpdatePayment(editingPayment.id, {
                  agreedAmount: parseFloat(
                    formData.get("agreedAmount") as string,
                  ),
