...
export default function WagesPage() {
  ...
  const applyFilters = () => {
    ...
    if (filters.status) {
      filtered = filtered.filter(w => w.status === filters.status.toUpperCase());
    }
  ...
  const handleCalculateWages = async (month: number, year: number, teacherId?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: errorData.error || 'Maaş hesaplaması başarısız' });
      }
    } catch (error) {
      console.error("Maaş hesaplama hatası:", error);
      setMessage({ type: 'error', text: 'Maaş hesaplaması sırasında hata oluştu' });
    } finally {
      ...
      <div className="rounded-lg bg-white p-6 shadow-card dark:bg-gray-dark">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <button
              onClick={() => setFilters({ month: "", year: "", status: "", teacherId: "" })}
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-dark dark:divide-gray-700">
              {filteredWages.map((wage) => (
                <tr key={wage.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
                        {wage.teacher.user.firstName} {wage.teacher.user.lastName}
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(wage.status)}`}>
