...
export default async function TeacherAttendancePage() {
  ...
  const attendanceStats = assignments.map((assignment) => {
    ...
    const attendanceCounts = assignment.class.lessons.reduce(
      (acc, lesson) => {
  ...
  const concernStudents = await Promise.all(
    assignments.flatMap(async (assignment) => {
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-dark dark:text-white">
                {attendanceStats.length > 0
                  ? Math.round(
                      attendanceStats.reduce(
                        (acc, stat) => acc + stat.attendanceRate,
                        0,
        ...
        <div className="p-6">
          <div className="space-y-6">
            {attendanceStats.map((stat) => (
              <div
                key={stat.assignment.id}
                className="border-b border-gray-100 pb-6 last:border-b-0 dark:border-gray-700"
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {stat.assignment.school.name} • {stat.totalStudents}{" "}
                      öğrenci • {stat.totalLessons} ders
                    </p>
                  </div>
                  <div className="flex space-x-3">
                    <Link
