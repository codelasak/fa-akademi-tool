        ...
        <div className="rounded-lg bg-white p-6 shadow-card dark:bg-gray-dark">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-dark dark:text-white">
                {lessons.reduce((sum, lesson) => sum + Number(lesson.hoursWorked), 0).toFixed(1)}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="rounded-full bg-red-100 p-3 dark:bg-red-900">
              <svg className="h-6 w-6 text-red-600 dark:text-red-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.736 4h4.018a2 2 0 01.485.06l3.76.94m-7 10v5a2 2 0 002 2h.096c.5 0 .905-.405.905-.904 0-.715.211-1.412.608-2.008L17 13m-5 1V9a2 2 0 012-2h1" />
              </svg>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6">
        {Object.entries(lessonsByMonth).length === 0 ? (
          ...
          Object.entries(lessonsByMonth).map(([month, monthLessons]) => (
            <div key={month} className="rounded-lg bg-white shadow-card dark:bg-gray-dark">
              <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                <h2 className="text-lg font-semibold text-dark dark:text-white">
                  {month} ({monthLessons.length} ders, {monthLessons.reduce((sum, lesson) => sum + Number(lesson.hoursWorked), 0).toFixed(1)} saat)
                </h2>
              </div>
              <div className="p-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="mt-2 grid grid-cols-1 gap-2 text-sm text-gray-600 dark:text-gray-400 md:grid-cols-3">
                            ...
                            <div>
                              <strong>Süre:</strong> {lesson.hoursWorked.toString()} saat
                            </div>
                            <div>
                              <strong>Yoklama:</strong> {lesson.attendance.filter((a: any) => a.status === "PRESENT").length}/{lesson.attendance.length} öğrenci
                            </div>
                          </div>

                          {lesson.topics.length > 0 && (
                            <div className="mt-2">
                              <strong className="text-sm text-gray-700 dark:text-gray-300">İşlenen Konular:</strong>
                          ...
                          {lesson.notes && (
                            <div className="mt-2">
                              <strong className="text-sm text-gray-700 dark:text-gray-300">Notlar:</strong>
                          ...
                          {lesson.isCancelled && lesson.cancellationReason && (
                            <div className="mt-2">
                              <strong className="text-sm text-red-700 dark:text-red-300">İptal Nedeni:</strong>
