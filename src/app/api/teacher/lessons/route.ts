...
const createLessonSchema = z.object({
  ...
  attendance: z.record(z.string(), z.object({
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (data.attendance) {
        ...
        const policy = await getEffectivePolicy(data.classId, assignment.schoolId);
        ...
        if (validAttendanceEntries.length > 0) {
          const attendanceData = validAttendanceEntries.map(([studentId, attendanceInfo]) => {
            let finalStatus = attendanceInfo.status as "PRESENT" | "ABSENT" | "LATE" | "EXCUSED";
            let appliedRules: string[] = [];
            
            // Apply policy-based late tolerance rules
            if (attendanceInfo.arrivalMinutes !== undefined && attendanceInfo.arrivalMinutes > 0) {
              if (attendanceInfo.arrivalMinutes > policy.lateToleranceMinutes) {
                finalStatus = "LATE";
                appliedRules.push(`Late: ${attendanceInfo.arrivalMinutes}min > ${policy.lateToleranceMinutes}min tolerance`);
              } else {
                finalStatus = "PRESENT";
                appliedRules.push(`Present: ${attendanceInfo.arrivalMinutes}min within ${policy.lateToleranceMinutes}min tolerance`);
              }
            }
            
            // Apply policy-based auto-excuse rules
            if (policy.autoExcuseEnabled && finalStatus === "ABSENT" && attendanceInfo.excuseReason) {
              const reasonLower = attendanceInfo.excuseReason.toLowerCase().trim();
              const matchingReason = policy.autoExcuseReasons.find(policyReason => 
              ...
              if (matchingReason) {
                finalStatus = "EXCUSED";
                appliedRules.push(`Auto-excused: "${attendanceInfo.excuseReason}" matches policy reason "${matchingReason}"`);
              }
            }
            ...
            return {
              ...
              notes: [
                ...
                ...appliedRules,
                attendanceInfo.excuseReason ? `Reason: ${attendanceInfo.excuseReason}` : null,
              ].filter(Boolean).join(" | "),
            };
          });
